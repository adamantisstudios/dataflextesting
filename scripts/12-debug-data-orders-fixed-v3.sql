...
SELECT 'Checking data_bundles table structure' as debug_step;
...
-- Test the exact query that the admin panel uses (without commission_rate for now)
...
SELECT 
    'Orders with missing agent' as issue, 
    COUNT(*) as count
FROM data_orders
LEFT JOIN agents ON data_orders.agent_id = agents.id
...
SELECT 
    'Orders with missing bundle' as issue, 
    COUNT(*) as count
FROM data_orders
LEFT JOIN data_bundles ON data_orders.bundle_id = data_bundles.id
